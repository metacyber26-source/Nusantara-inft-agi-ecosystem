...
import { AlertCircle, CheckCircle, Loader2, Zap, Boxes } from "lucide-react"
...
interface Generate3DInftPaymentButtonProps {
...
export function Generate3DInftPaymentButton({
  variant = "default",
  size = "default",
  fullWidth = false,
  compact = false,
  onSuccess,
}: Generate3DInftPaymentButtonProps) {
  ...
  // Use 3D iNFT Generator product ID
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a2ca1593ea3bac3597ca111
  )

  // Get quantity from restored purchases
  const purchaseInfo = restoredPurchases?.find(
    (p) => p.productId === PRODUCT_CONFIG.PRODUCT_6a2ca1593ea3bac3597ca111
  )
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      if (purchaseResult.ok) {
        setResult({
          type: "success",
          message: "Purchase successful! Your 3D iNFT generator is ready to use.",
          details: {
        ...
        >
          ...
          <div className="relative space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-muted-foreground">AI-driven 3D generation</p>
                </div>
              </div>
            ...
            <p className="text-xs text-muted-foreground">
              Auto-rig, mint & trade for Web3 games & metaverse
            </p>
          </div>
        </button>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Create AI-driven interactive 3D iNFTs with automatic rigging
              </DialogDescription>
            </DialogHeader>
                ...
                <>
                  ...
                  {quantity > 0 && (
                    <Alert className="border-blue-500/50 bg-blue-500/10">
                      ...
                      <AlertDescription className="text-blue-700 dark:text-blue-400">
                        You have already purchased this. Click the button to generate.
                      </AlertDescription>
                    </Alert>
                  )}
                  ...
                  <ul className="space-y-2 text-xs">
                    <li className="flex gap-2">
                      ...
                      <span>AI-driven 3D iNFT generation from text/2D prompts</span>
                    </li>
                    <li className="flex gap-2">
                      <span className="text-primary">✓</span>
                      <span>Automatic rigging to GLTF/GLB/FBX/VRM formats</span>
                    </li>
                    <li className="flex gap-2">
                      <span className="text-primary">✓</span>
                      <span>Multilingual voice & text support (EN/ID/JV)</span>
                    </li>
                    <li className="flex gap-2">
                      <span className="text-primary">✓</span>
                      <span>Optimized for Web3 games & metaverse platforms</span>
                    </li>
                    <li className="flex gap-2">
                      <span className="text-primary">✓</span>
                      <span>Fair ecosystem revenue-sharing splits</span>
                    </li>
                  </ul>
                </>
              )}
  ...
  // Standard button version for hero/CTA placement
  return (
    <>
      <Button
        onClick={() => {
      ...
      >
        {loading ? (
          ...
          <>
            ...
            Generate 3D iNFT · {product.price_in_pi} Pi
          </>
        )}
      </Button>
